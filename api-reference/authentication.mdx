...
<Note>
  **Using MCP instead?** The [MCP server](/api-reference/mcp#authentication) uses browser-based OAuth, not API keys — each user signs in once from their MCP client and a session token is issued automatically. API keys (below) are for REST and server-to-server use.
</Note>
